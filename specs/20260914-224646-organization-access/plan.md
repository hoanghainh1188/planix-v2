...
(migration) · `planix_app` (ứng dụng, RLS) · `planix_platform` (`/platform/*`) — research R3
...
([contracts/api.md](contracts/api.md)), 12 bảng ([data-model.md](data-model.md)), ~10 màn hình web
...
| **III. Bảo mật mặc định từ chối** | RBAC ∧ RACI; field-level API; cô lập tenant có test; không log dữ liệu nhạy cảm | ✅ PASS — FR-001–003, 016, 022–025, 028 | ✅ PASS — `decide` không nhánh Admin, action lạ → Deny; RLS + `TenantContext` (không nhận `organizationId` từ client) với 4 lối truy cập hẹp có test (R3); interceptor xoá khoá field nhạy cảm cả ở lỗi/audit; token/mật khẩu băm; log ứng dụng che dữ liệu nhạy cảm (R14) ([authorization.md](contracts/authorization.md)) |
...
| **VI. Thuật ngữ nhất quán** | Tra glossary; tách khái niệm trùng | ✅ PASS — định danh theo glossary (`organization`, `projectMember`, `accountable`…) | ✅ PASS — thuật ngữ mới (`platformOperator`, `organizationMembership`, `membershipRole`, `organizationInvitation`, `authSession`, `activeOrganization`, `passwordResetToken`, `auditEntry`, `sensitiveFieldPolicy`, giá trị role) được append glossary ở **task đầu Phase 2, trước khi code dùng tên** (sửa sau `/speckit-analyze` D1); `glossary-steward` rà lại ở bước 12 |
...
└── src/
    ...
    │   ├── logging/                 # AppLogger + redaction (R14)
    │   └── errors/                  # mã lỗi → HTTP
    ├── features/organization-access/
...
| 3 role DB (`planix_owner`, `planix_app`, `planix_platform`) + 4 lối truy cập vượt tổ chức hẹp (R3) | Đăng nhập, tra lời mời chưa có phiên và `/platform/*` không có tổ chức đang hoạt động; RLS vẫn phải giữ nguyên ở các luồng này | Cấp `BYPASSRLS` cho ứng dụng ở các luồng đặc biệt: mất lớp chắn thứ hai đúng nơi rủi ro nhất |
