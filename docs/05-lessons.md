...
| 2026-09-15 | Rule `planix/no-literal-string` chỉ soi chuỗi **nằm trực tiếp trong JSX** (text, `{'…'}`, nhánh `? :`, `&&`/`\|\|`/`??`, thuộc tính hiển thị): chuỗi đặt trong biến, object literal hay mảng rồi truyền vào prop (`options={[{ label: 'Open' }]}`, `const title = 'Settings'`) **không bị bắt** | `005-organization-access` · code review Phase 10 | Không dựng nhãn hiển thị trong object/biến ngoài JSX; gọi `t(...)` ngay tại chỗ dựng dữ liệu; review tay các prop nhận object |
| 2026-09-15 | `autocannon` 8 **không có p95** trong kết quả (chỉ p90, p97.5…) và không kèm type TypeScript; `process.exit` giữa chừng bỏ qua dọn container | `005-organization-access` · Phase 11 T123 | Tự tính p95 (nearest-rank, có test) từ sự kiện `response`; khai báo type tối thiểu `perf/autocannon.d.ts`; báo kết quả bằng `process.exitCode` sau khi dừng container |
| 2026-09-15 | Lỗi của body parser Express (413 `entity.too.large`) **không phải `HttpException`** → `DomainErrorFilter` trả 500 `INTERNAL_ERROR`; `express.disable('x-powered-by')` là thừa khi đã có `helmet()` (helmet tự gỡ header) | `005-organization-access` · Phase 11 T125 | Nhận diện theo `error.type === 'entity.too.large'` → 413 `PAYLOAD_TOO_LARGE`; đặt helmet + body limit trong `configureApp` để test tích hợp dùng đúng cấu hình production |
| 2026-09-15 | Test/E2E/perf **seed tài khoản Platform Operator bằng SQL** nên che mất lỗ hổng: cài đặt mới không có đường nào tạo Operator đầu tiên (tài khoản chỉ sinh từ lời mời, lời mời cần tổ chức, tổ chức cần Operator) — chỉ lộ khi chạy quickstart trên DB trống | `005-organization-access` · Phase 11 T127 | Chạy quickstart từ DB trống trước khi đóng feature; bootstrap bằng `ops:grant-operator --create` (decision `2026-09-15-005-first-platform-operator`); rate limit IP của `/auth/*`, `/invitations/*` nằm trong bộ nhớ — chạy lại kịch bản trên dev thì khởi động lại server |
